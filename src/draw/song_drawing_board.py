...
class SongDrawingBoard(DrawingBoard):
    def __init__(self, song_data):
        ...
        main_img_path = os.path.join(context["assets_path"], "song", "base", f"{self.level_index}.png")
    ...
    def draw_song_type(self, position=(6, 3)):
        ...
        """
        type_img_path = os.path.join(self.maimai_assets_folder, "song", f"{self._type.lower()}.png")
    ...
    def draw_song_rank(self, position=(130, 0), font_size=18):
        ...
        draw_ra_plate.text((12, 7), str(self.ra), font=font_ra, fill=(255, 200, 0), stroke_width=2, stroke_fill=(0, 0, 0),
                           align='center')
    ...
    def draw_song_ds(self, position=(12, 7), font_size=19):
        ...
        """
        font_ds = self.get_font(self.en_font, font_size=19)
        draw_base = ImageDraw.Draw(self.main_img)
        draw_base.text(position, str(self.ds), font=self.get_font(font_size), fill=(255, 255, 255), stroke_width=2, stroke_fill=(0, 0, 0),
                       align='center')
    ...
    def draw_song_rate(self, position=(10, 210), target_height=32):
        ...
        """
        if self.rate:
            rate_img_path = os.path.join(self.assets_path, "song", "rank", f"{self.rate}.png")
    ...
    def draw_song_achievement(self, position=(75, 215), font_size=20):
        ...
        """
        if self.achievement:
            self.main_draw.text(position, "{:.4f}".format(self.achievements), font=self.get_font(font_size), fill=(255, 255, 255),
    ...
    def draw_song_title(self, position=(20, 25), font_size=16):
        ...
        self.main_draw.text(position, wrapped_text, font=self.get_font(font_size), fill=(255, 255, 255), stroke_width=2,
                           stroke_fill=(0, 0, 0))
    ...
    def draw_song_badge(self, position=(135, 125)):
        ...
        """
        if self.fc:
            fc_img_path = os.path.join(self.assets_path, "song", "badges", f"{self.fc}_s.png")
            fc_img = Image.open(fc_img_path)
            self.paste(fc_img, position, fc_img)
            position[0] -= 35

        if self.fs:
            fs_img_path = os.path.join(self.maimai_assets_folder, "song", "badges", f"{self.fs}_s.png")
            fs_img = Image.open(fs_img_path)
            self.paste(fs_img, position, fs_img)
